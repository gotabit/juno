...
LD_FLAGS = -X github.com/gotabit/juno/v5/cmd.Version=$(VERSION) \
	-X github.com/gotabit/juno/v5/cmd.Commit=$(COMMIT)
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -name '*.pb.go' -not -path "./venv" | xargs goimports -w -local github.com/gotabit/juno
.PHONY: format
